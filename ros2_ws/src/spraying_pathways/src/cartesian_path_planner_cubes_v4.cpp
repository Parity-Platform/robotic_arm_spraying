...
int main(int argc, char** argv) {
  ...
  // New Code --> Define your robot base position in world (same as spawn)
    ...
    for (int j = 0; j < cols; ++j) {
      ...
      for (int cx = 0; cx < N; ++cx) {
        for (int cy = 0; cy < N; ++cy) {
          ...
          double dx_c = cx - (N - 1) / 2.0;
          double dy_c = cy - (N - 1) / 2.0;
          double dist_norm = std::sqrt(dx_c * dx_c + dy_c * dy_c) / ((N - 1) / 2.0);
          double prob_z1 = std::clamp(0.9 * (1.0 - dist_norm), 0.0, 0.9);
          double rand_val = (std::rand() % 10000) / 10000.0;
          double height = (rand_val < prob_z1)
            ? max_height
            : random_height_min + ((std::rand() % 10000) / 10000.0) * (random_height_max - random_height_min);
          double z_offset = z_base + height / 2.0;
          geometry_msgs::msg::Pose pose_cube;
          pose_cube.position.x = x_pos;
          pose_cube.position.y = y_pos;
          pose_cube.position.z = z_offset;
          pose_cube.orientation = orientation;
          cubes.push_back(Cube{pose_cube, height, count++});
        }
      }
    }
  }
