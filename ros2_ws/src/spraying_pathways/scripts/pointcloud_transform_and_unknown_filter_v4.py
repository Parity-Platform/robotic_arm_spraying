...
class CombinedPipelineNode(Node):
    ...
    def __init__(self, world_file):
        ...
        self.pub_camera_ee_all = self.create_publisher(PointCloud2, "/camera_ee_all_points", 10)
...
################################################################################################################
        ...
        theta_z_e = math.radians(-90)
        R_z_e = np.array([[math.cos(theta_z_e), -math.sin(theta_z_e), 0],
                        [math.sin(theta_z_e), math.cos(theta_z_e), 0],
                        [0, 0, 1]], dtype=np.float32)
        theta_y_e = math.radians(90)
        R_y_e = np.array([[math.cos(theta_y_e), 0, math.sin(theta_y_e)],
                        [0, 1, 0],
                        [-math.sin(theta_y_e), 0, math.cos(theta_y_e)]], dtype=np.float32)
        theta_x_e = math.radians(-90)
        R_x_e = np.array([[1, 0, 0],
                        [0, math.cos(theta_x_e), -math.sin(theta_x_e)],
                        [0, math.sin(theta_x_e),  math.cos(theta_x_e)]], dtype=np.float32)
        self.R_initial_e = R_y_e @ R_z_e
        #self.R_initial_e = R_x_e @ R_y_e

        theta_z1_e_custom = math.radians(-90)
        R1_e_custom = np.array([[math.cos(theta_z1_e_custom), -math.sin(theta_z1_e_custom), 0],
                        [math.sin(theta_z1_e_custom), math.cos(theta_z1_e_custom), 0],
                        [0, 0, 1]], dtype=np.float32)

        theta_y2_e_custom = math.radians(0)
        R2_e_custom = np.array([[math.cos(theta_y2_e_custom), 0, math.sin(theta_y2_e_custom)],
                        [0, 1, 0],
                        [-math.sin(theta_y2_e_custom), 0, math.cos(theta_y2_e_custom)]], dtype=np.float32)

        theta_x3_e_custom = math.radians(90)
        R3_e_custom = np.array([[1, 0, 0],
                        [0, math.cos(theta_x3_e_custom), -math.sin(theta_x3_e_custom)],
                        [0, math.sin(theta_x3_e_custom),  math.cos(theta_x3_e_custom)]], dtype=np.float32)

        #self.R_custom = R2_custom
        self.R_e_custom = R1_e_custom @ R3_e_custom
        self.translation_e = np.array([-0.06, 0.0, -0.03], dtype=np.float32)
        ...
        # Create it last so that, if spin happens above, the callback doesn’t run early.
        self.subscription_raw = self.create_subscription(
            PointCloud2, "/wrist3_cam/points", self.depth_pointcloud_ee_callback, 10
        )
    ...
    def get_link_base_link_pose(self, link_name):
        try:
    ...
    def depth_pointcloud_ee_callback(self, msg):
        # Robust NumPy read (structured or Nx3)
        pts = None
        try:
            arr = pc2.read_points_numpy(msg, field_names=("x", "y", "z"))
        ...
        # Unified transform
        # optical -> camera_link
        #pts_cam  = (self.R_initial_e @ pts.T).T

        # camera_link -> wrist_3_link  (ΠΡΩΤΑ rotate, ΜΕΤΑ add)
        #pts_base = (self.R_e_custom  @ pts_cam.T).T + self.translation_e
        pts_base = pts + self.translation_e

        pts = pts_base

        trans, quat = self.get_link_base_link_pose('wrist_3_link')

        if trans is not None:
            # R,t που μεταφέρουν p_wrist -> p_base:  p_base = R * p_wrist + t
            R_wrist_to_base = quaternion_matrix(quat).astype(np.float32)[:3, :3]
            pts_base_link   = (R_wrist_to_base @ pts_base.T).T + trans
            pts = pts_base_link
        else:
            self.get_logger().warn("[EE cam] TF wrist_3_link->base_link not available (latest). Keeping wrist_3_link frame.")
            pts = pts_base
        ...
        """hdr_all = msg.header
        hdr_all.frame_id = "base_link"
        self.pub_camera_ee_all.publish(pc2.create_cloud_xyz32(hdr_all, pts[m].tolist()))"""

        hdr = Header()
        hdr.stamp = msg.header.stamp 
        hdr.frame_id = "base_link"
        self.pub_camera_ee_all.publish(pc2.create_cloud_xyz32(hdr, pts[m].tolist()))
